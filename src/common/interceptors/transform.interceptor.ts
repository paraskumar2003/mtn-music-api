    ...
    implements NestInterceptor<T, ApiResponse<T>>
{
     ...
     */
    private sanitize<T>(value: T, seen = new WeakSet()): T {
        if (Array.isArray(value)) {
            return value.map(v => this.sanitize(v, seen)) as any;
        }
        ...
        if (typeof value === 'object' && value !== null) {
            if (seen.has(value as any)) {
                return undefined as any; // break circular reference
            }
            ...
            for (const key in value) {
                if (!this.sensitiveFields.includes(key)) {
                    clean[key] = this.sanitize((value as any)[key], seen);
                }
            }
            return clean;
        }
