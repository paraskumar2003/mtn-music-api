...
export class ReportPdfService {
    ...
    private strength_text: Record<string, any> = {
        visual: {
            ...
            excellent:
                'You rapidly convert abstract imagery into structured mental models with precision. Visual cues are interpreted holistically and strategically. Your thinking reflects clarity, foresight, and compositional awareness. This is a dominant cognitive strength.',
        },
        ...
        auditory: {
            ...
            excellent:
                'You instinctively layer sound, rhythm, and tone onto visual stimuli. Visuals are experienced as dynamic sequences rather than static images. This enhances storytelling and anticipation. Auditory processing is highly integrated.',
        },
        ...
        rhythmic: {
            ...
            excellent:
                'You immediately perceive rhythm, cycles, and underlying systems within imagery. Patterns guide your interpretation and decision-making. This reflects advanced systems thinking. Predictive reasoning is highly developed.',
        },
        ...
        subconscious: {
            ...
            excellent:
                'You instinctively access abstract, symbolic, and subconscious layers of meaning. Visuals trigger intuitive understanding beyond explicit content. Interpretations are rich, nuanced, and multidimensional.',
        },
    };
    ...
    private async generateAttachments(attachmentsInput: any[]): Promise<any[]> {
        const attachments: any[] = [];

        for (const item of attachmentsInput) {
            // Case 1: In-memory Buffer attachment
    ...
    private async sendMail(data: SendMailDto): Promise<void> {
        try {
            const msg: any = {
                to: data?.to,
                from: data.from,
            ...
            sgMail
                ...
                .then((response: any) => {
                    this.logger.info(
                        `Email sent successfully to ${data.to}`,
                        data.from,
                        { response },
                    );
                })
                .catch(async error => {
                    this.logger.error(
                        `Email sending failed to ${data.to}`,
                        data.from,
                        { error },
                    );
                });
    ...
    private sectionTitle(doc: typeof PDFDocument, title: string) {
        doc.moveDown(1.2).fontSize(14).text(title).moveDown(0.5).fontSize(11);
    }

    private kv(doc: typeof PDFDocument, key: string, value: string) {
    ...
    async generatePdf(data: NeuroprofilingMailVariablesDto): Promise<Buffer> {
        ...
        doc.fontSize(11).text(
            `This report is generated based on your responses to a series of visual images presented during the Neuroprofiling Visual Assessment.
...
There are no right or wrong answers. This assessment highlights your natural thinking and interpretation style.`,
            { align: 'justify' },
        );
        ...
        this.kv(
            doc,
            'Subconscious / Abstract Insight',
            `${data.mix_subconscious}%`,
        );
        ...
        this.subSection(
            ...
            this.strength_text.rhythmic[this.getLevel(data.mix_rhythmic)],
        );
        this.subSection(
            doc,
...
This report does not constitute a medical, psychological, or clinical diagnosis and should not be used as a substitute for professional evaluation.`,
            { align: 'justify' },
        );
